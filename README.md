...
Note that it is not necessary to reproduce the hierarchy of classes in a single codebase by combining the branches. You can simply modify your previous version to integrate the changes shown in Project 2, or select each of the branches in VSCode. The hierarchy of classes is done for pedagogical purposes to highlight the changes from one version to the next making knowledge more bit-wise. This incremental approach will be followed throughout the next sections.
